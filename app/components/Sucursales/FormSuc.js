 ...
 que contendrán la información de los campos del formulario*/
  ...
  const subirImagenesStorage = async () => {
    ...
    de carga de archivos a servidor (nuestro storage de firebase)*/
        ...
        /*almacenamos casa imagen en el storage sucursales de firebase,
    el nombre de cada imagen será generado por el random de uuid*/
        const ref = firebase.storage().ref("sucursales").child(uuid());
        //al procesar el envío iniciamos una procesa que atrapara el objeto enviado
        await ref.put(blob).then(async (resultado) => {
          /*recuperamos el nombre de cada imagen almacenada para referenciarlo al
    insertar la sucursal*/
          await firebase
            ...
            .getDownloadURL()
            .then((urlFoto) => {
              //cada referencia de las imágenes almacenadas las cargamos en el arreglo
...
//Crearemos un botón para subir una imagen
function SubirImagen(propiedades) {
  ...
  const seleccionar = async () => {
    ...
    if (resultado === "denied") {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        allowsEditing: true,
        //resolución estándar SD, marcará el tamaño de la imagen
        aspect: [4, 3],
      });
      if (result.cancelled) {
  ...
  const eliminarImagen = (imagen) => {
    //Recibimos la imagen a eliminar
